...
const build = (node, startingPosition, direction, visualise, angle, points = []) => {
    ...
    if (node.children.hasOwnProperty(key)) {
      ...
      if (childNode) {
        if (typeof childNode.children !== 'undefined') {
          ...
          build(childNode, endPosition, newDirection, visualise, angle, points)
...
export default function ({ output, n_tx }, material) {
  ...
  const points = build(tree, startingPosition, direction, false, angle)
  ...
  let blockMesh = new THREE.Mesh(boundingBoxGeometry, material.clone())
