...
void main() {
	...
	/*vec3 r = refract( normalize( outgoingLight.rgb ), worldNormal, 0.5 );
	vec3 g = refract( normalize( outgoingLight.rgb ), worldNormal, 0.4 );
	vec3 b = refract( normalize( outgoingLight.rgb ), worldNormal, 0.3 );*/

	//vec3 finalColor = outgoingLight + snoise(vec3(r.r, g.g, b.b)) * snoise(vNormal.rgb * 10.0);
	//vec3 finalColor = outgoingLight + snoise(outgoingLight.rgb) * snoise(vNormal.rgb * 10.0);
