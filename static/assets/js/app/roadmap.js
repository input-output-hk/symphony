...
function rm_item(item){
  ...
  var time_cls = 'notime';
    ...
    if(item.gsx$category.$t == 'Milestone'){
      if(rm_month[date_arr[0]*1] != undefined){
        time = '<span class="time">'+date_arr[1]+' '+rm_month[date_arr[0]*1]+' <span class="year">'+date_arr[2]+'</span></span>';
      }
    }else{
      if(q[date_arr[0]] != undefined){
        time = '<span class="time">Q'+q[date_arr[0]]+' <span class="year">'+date_arr[2]+'</span></span>';
      }
    }
    time_cls = 'time';
	}
